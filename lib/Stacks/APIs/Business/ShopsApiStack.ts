...
export class ShopApiStack extends cdk.NestedStack {
  constructor(scope: Construct, id: string, props: UsersApiStackProps) {
    ...
    // Get Shop API
    const getShopLambda = new nodejs.NodejsFunction(this, "my-handler", {
      runtime: lambda.Runtime.NODEJS_20_X,
    ...
    props.encryptionKey.grantEncryptDecrypt(getShopLambda);

    // API Gateway integration
    const getShopApi = shopApi.addResource("getShop");
    ...
    props.encryptionKey.grantEncryptDecrypt(discoverShopsLambda);
